...
class ThemeManager:
    ...
    DARK_THEME = {
        ...
        'html_bg': '#1E1E1E',
        'html_text': '#FFFFFF',
        'html_table_border': '#555',
        'html_table_header_bg': '#007ACC',
        'html_row_hover': '#3E3E42',
    }
    ...
    LIGHT_THEME = {
        ...
        'html_bg': '#FFFFFF',
        'html_text': '#000000',
        'html_table_border': '#DDD',
        'html_table_header_bg': '#007ACC',
        'html_row_hover': '#F5F5F5',
    }
...
class ProfileManager:
    ...
    PROFILES_FILE = "scan_profiles.json"
...
class ScanWorker(QThread):
    ...
    def __init__(self, settings):
        ...
        self.active_ips = set()
        self.process = None
        self.port_data = []  # داده‌های پورت برای جدول و گزارش
                    ...
                    if self.process.returncode == 0:
                        if os.path.exists(temp_output):
                            with open(temp_output, 'r', encoding='utf-8') as f:
                                content = f.read()
                                # پشتیبانی از خطوطی با قالب: "Nmap scan report for host (x.x.x.x)" و همچنین "Nmap scan report for x.x.x.x"
                                ips_found = []
    ...
    def generate_charts(self, output_dir):
        ...
        try:
            ...
            if port_states:
                ...
                plt.pie(port_states.values(), labels=list(port_states.keys()), autopct='%1.1f%%',
                        textprops={'fontproperties': font_prop})
                plt.title("توزیع وضعیت پورت‌ها", fontproperties=font_prop)
            ...
            if protocol_counts:
                ...
                plt.title("استفاده از پروتکل‌ها", fontproperties=font_prop)
                plt.xlabel("پروتکل", fontproperties=font_prop)
                plt.ylabel("تعداد", fontproperties=font_prop)
            ...
            if open_ports_per_ip:
                ...
                plt.title("تعداد پورت‌های باز بر اساس آی‌پی", fontproperties=font_prop)
                plt.xlabel("آی‌پی", fontproperties=font_prop)
                plt.ylabel("تعداد پورت‌های باز", fontproperties=font_prop)
                plt.tight_layout()
...
class ReportsTab(QWidget):
    ...
    def update_charts(self):
        ...
        try:
            ...
            if port_states:
                ax.pie(list(port_states.values()), labels=list(port_states.keys()), autopct='%1.1f%%',
                       textprops={'fontproperties': font_prop})
                ax.set_title("توزیع وضعیت پورت‌ها", fontproperties=font_prop)
            self.pie_chart.draw()
            ...
            if protocol_counts:
                ...
                ax.set_title("استفاده از پروتکل‌ها", fontproperties=font_prop)
                ax.set_xlabel("پروتکل", fontproperties=font_prop)
                ax.set_ylabel("تعداد", fontproperties=font_prop)
            self.bar_chart.draw()
            ...
            if open_ports_per_ip:
                ...
                ax.set_title("تعداد پورت‌های باز بر اساس آی‌پی", fontproperties=font_prop)
                ax.set_xlabel("آی‌پی", fontproperties=font_prop)
                ax.set_ylabel("تعداد پورت‌های باز", fontproperties=font_prop)
                plt.tight_layout()
    ...
    def export_charts(self):
        ...
        try:
            ...
            if os.path.exists(pie_path) and os.path.exists(bar_path) and os.path.exists(ports_path):
                save_dir = QFileDialog.getExistingDirectory(self, "انتخاب پوشه برای ذخیره نمودارها")
                if save_dir:
                    import shutil
                    shutil.copy2(pie_path, os.path.join(save_dir, "port_states_pie.png"))
                    shutil.copy2(bar_path, os.path.join(save_dir, "protocol_usage_bar.png"))
                    shutil.copy2(ports_path, os.path.join(save_dir, "open_ports_per_ip.png"))
